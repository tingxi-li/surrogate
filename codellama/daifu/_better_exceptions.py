...
class ExceptionFormatter:
    ...
    def __init__(
        ...
        self.latest_meta_info = {}
        self.len_considered_frames = None
    ...
    def _extract_frames(self, tb, is_first, *, limit=None, from_decorator=False):
        ...
        def get_info(frame, lineno):
            ...
            f_code = frame.f_code
            return filename, lineno, function, source, f_code
        ...
        if (self._backtrace and is_first) or get_parent_only:
            ...
            if infos and not get_parent_only:
                (filename, lineno, function, source, f_code), frame = infos[-1]
                function += self._catch_point_identifier
                infos[-1] = ((filename, lineno, function, source, f_code), frame)
        ...
        self.latest_meta_info = []
        is_first = True
        for (filename, lineno, function, source, f_code), frame in infos[1:]:
            if 'site-packages' in filename or (not filename.endswith('.py') and 'ipython' not in filename):
                if self.len_considered_frames is None:
                    self.len_considered_frames = len(self.latest_meta_info)
            final_source = source
            if source:
            ...
            if is_first:
                ...
                function = self.first_function
                is_first = False
            frames.append((filename, lineno, function, source))
            self.latest_meta_info.append({'filename': filename, 'lineno': lineno, 'function': function, 'f_code': f_code})
